    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="offShelves(props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="onShelves(props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
        ...
        methods: {
            ...
            deleteGoods(goods){
                this.$message.confirm('此操作将永久删除数据，是否继续?', '提示', {
                    ...
                    this.$http.delete("/item/goods/"+goods.id)
                        .then(() => {
            ...
            onShelves(spuId){
                this.$http({
                    method: 'post',
                    url: '/item/spu/onshelves',
                    data: this.$qs.stringify({'spuId':spuId})
                }).then(() => {
            ...
            offShelves(spuId){
                // this.$http.post("/item/spu/offshelves",{
                //     spuId: spuId
                // }).then(()=>{
