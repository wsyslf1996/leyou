  ...
  <div>
    <v-list-tile
      ...
      <v-list-tile-content>
        ...
        <v-dialog v-model="beginEdit" width="300" height="200">
          <v-card>
            <v-card-text>
              <v-text-field :label="tipsTitle" :ref="tmp.id" v-model="tmp.name"/>
            </v-card-text>
            <v-card-actions>
              <v-spacer/>
              <v-btn color="blue darken-1" flat @click.native="beginEdit=false">取消</v-btn>
              <v-btn color="blue darken-1" flat @click.native="afterEdit">保存</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editChild() {
                console.log("开始修改");
                Object.assign(this.tmp, this.model);
                this.beginEdit = true;
                // this.$nextTick(() => this.$refs[this.model.id].focus());
            },
            afterEdit() {
                console.log("修改完毕");
                // this.model.name=this.tmp.name;
                this.beginEdit = false;
                this.handleEdit(this.tmp.id, this.tmp.name); //触发父组件事件
            },
            handleAdd(node) {
